...
import { deserializeKeys, prefixBounds, serializeKeys } from "./keys.mjs";

/**
 * Property-based tests for the order-preserving key codec. Unlike
 * keys.test.mjs's example-based regression tests (which pin specific known
 * cases, including the exact N1 ordering bug), these generate many random
 * composite keys per run and check invariants that must hold for *any* valid
 * key — round-tripping, cross-type/same-type ordering, and prefix
 * containment — using a comparator written independently from keys.mjs's own
 * encode/decode logic (see `referenceCompareKeys` below), so a bug in the
 * implementation can't also be baked into the check.
 */
...
// disagree exactly in cases like these, which is the property worth fuzzing
// (see kv.md: "Strings sort by UTF-8 byte order").
const CHAR_POOL = [
	..."abcXYZ019 _-",
	..."éñüçÉÑ",
	..."中文字符",
...
// --- Independent reference comparator (documented rules, not keys.mjs's own encoding) ---
...
function normalizeForRoundTrip(key) {
	// The only lossy, documented normalization: -0 collapses to 0 for numbers.
	return key.map((s) => (typeof s === "number" && Object.is(s, -0) ? 0 : s));
}

describe("keys.mjs property tests (fuzz)", () => {
